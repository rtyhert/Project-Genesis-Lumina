...
from upage_bridge.bridge_server import BridgeServer, BridgeMessage

try:
    from upage_proto.build import upage_pb2 as pb2
    from upage_proto.build import upage_pb2_grpc as pb2_grpc
except ImportError:
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).parent.parent.parent / "upage-proto" / "build"))
    import upage_pb2 as pb2
    import upage_pb2_grpc as pb2_grpc

log = logging.getLogger("upage.grpc")
...
class VirtualHumanServicer(pb2_grpc.VirtualHumanServicer):
    ...
    async def StreamAudio(
        ...
        try:
            from upage_py.src.tts_engine import TTSEngine
            tts = TTSEngine({
                "engine": "edge-tts",
    ...
    async def ProcessAudioStream(
        ...
        try:
            async for chunk in request_iterator:
                ...
                if chunk.is_final:
                    break
            from upage_py.src.stt_engine import STTEngine
    ...
    async def SendEmotion(
        ...
        try:
            ...
            from upage_py.src.neuro_engine import NeuroEngine
