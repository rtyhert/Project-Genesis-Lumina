...
void LogInit() {
    std::cout << "[" << Timestamp() << "] [Main] Lumina-cpp v0.1.0 starting..." << std::endl;
}
...
void ConnectionMonitor(lumina::BridgeClient& bridge) {
    int fail_count = 0;
    while (g_running.load()) {
        if (!bridge.IsConnected()) {
            ++fail_count;
            std::cerr << "[" << Timestamp() << "] [Main] Connection lost (attempt "
                      << fail_count << ")" << std::endl;
        } else {
            fail_count = 0;
        }
...
void InteractiveLoop(lumina::BridgeClient& bridge, lumina::AudioEngine& audio) {
    ...
    while (g_running.load()) {
        ...
        if (line.rfind("/vol ", 0) == 0) {
            try {
                ...
                std::cout << "[Main] Volume set to " << static_cast<int>(vol * 100)
                          << "%" << std::endl;
            } catch (...) {
...
int main(int argc, char* argv[]) {
    ...
    lumina::BridgeClient bridge(server_addr);
    lumina::AudioEngine audio;
    lumina::Renderer renderer;
    lumina::InputHandler input_handler;
    lumina::Live2DModel live2d;

    if (!renderer.Init(opts.width, opts.height, "Lumina Virtual Human")) {
    ...
    renderer.SetModel(std::make_unique<lumina::Live2DModel>(std::move(live2d)));
