...
BridgeClient::BridgeClient(const std::string& server_addr)
    ...
    std::cout << "[Lumina-Bridge] Connected to " << server_addr << std::endl;
}
...
bool BridgeClient::EnsureConnection() {
    ...
    std::cerr << "[Lumina-Bridge] Connection lost, reconnecting in "
...
ChatOutput BridgeClient::ProcessChat(const ChatInput& input) {
    ...
    for (int retry = 0; retry < 3; ++retry) {
        ...
        if (status.error_code() == grpc::StatusCode::DEADLINE_EXCEEDED) {
            std::cerr << "[Lumina-Bridge] ProcessChat timeout" << std::endl;
        } else {
            std::cerr << "[Lumina-Bridge] ProcessChat error: "
...
void BridgeClient::StreamAudio(const std::string& text, AudioCallback on_audio) {
    ...
    if (!status.ok()) {
        std::cerr << "[Lumina-Bridge] StreamAudio error: "
                  << status.error_message() << std::endl;
    }
}
...
bool BridgeClient::TriggerAction(const std::string& type, const std::string& emotion,
    ...
    if (status.ok() && resp.success()) {
        std::cout << "[Lumina-Bridge] Action triggered: " << resp.animation_id()
                  << " (" << resp.duration() << "s)" << std::endl;
        return true;
    }
    std::cerr << "[Lumina-Bridge] TriggerAction failed: "
...
bool BridgeClient::SendEmotion(const std::string& session_id,
    ...
    req.mutable_emotion()->set_category(
        static_cast<EmotionCategory>(category));
    ...
    if (status.ok() && resp.success()) {
        std::cout << "[Lumina-Bridge] Emotion sent: " << resp.transition_animation()
                  << std::endl;
        return true;
    }
    std::cerr << "[Lumina-Bridge] SendEmotion failed: "
              << status.error_message() << std::endl;
    return false;
}
...
bool BridgeClient::Live2DControl(const Live2DControlArgs& args) {
    ...
    if (status.ok() && resp.success()) {
        std::cout << "[Lumina-Bridge] Live2D: " << resp.animation_id()
                  << " (" << resp.transition_duration() << "s)" << std::endl;
        return true;
    }
    std::cerr << "[Lumina-Bridge] Live2DControl failed: "
...
void BridgeClient::StreamStatus(const std::string& client_id,
    ...
    Status status = reader->Finish();
    if (!status.ok()) {
        std::cerr << "[Lumina-Bridge] StreamStatus ended: "
                  << status.error_message() << std::endl;
    }
}
...
bool BridgeClient::SendGift(const std::string& stream_id,
    ...
    if (status.ok() && resp.success()) {
        std::cout << "[Lumina-Bridge] Gift response: " << resp.thank_you_text()
                  << std::endl;
        return true;
    }
    std::cerr << "[Lumina-Bridge] SendGift failed: "
              << status.error_message() << std::endl;
    return false;
}
...
void BridgeClient::StreamLiveStatus(const std::string& stream_id,
    ...
    Status status = reader->Finish();
    if (!status.ok()) {
        std::cerr << "[Lumina-Bridge] StreamLiveStatus ended: "
                  << status.error_message() << std::endl;
    }
}
